  ...
  const loadNewEpisodes = async () => {
    ...
    function filterByDuration(episode: EpisodeData) {
      ...
      if (episode.podcastUrl in podcastSettings) {
        const episodePodcastSettings = getPodcastSettings(
          episode.podcastUrl,
          podcastSettings
        );
        ...
        if (episodePodcastSettings.filter.duration.min > 0) {
          result =
            result &&
            episode.duration >= episodePodcastSettings.filter.duration.min;
        }

        if (episodePodcastSettings.filter.duration.max > 0) {
          result =
            result &&
            episode.duration <= episodePodcastSettings.filter.duration.max;
        }
      }
      return result;
    }
...

      {
        ...
        queue.queue.length === 0 && newEpisodes.length === 0 && (
          <div className="flex flex-col items-center gap-4 p-2 px-4">
            ...
            <h1 className="text-center text-lg text-primary-3">
              {t("welcome_message")}
            </h1>
          </div>
        )
