...
export function useMisc(db: Database) {
  ...
  const getSyncKey = useCallback(async function (): Promise<string | undefined> {
    return await getMiscKey('syncKey')
  }, [db])

  const setSyncKey = useCallback(async function (key: string) {
    return await setMiscValue('syncKey', key)
  }, [db])

  const getLastSync = useCallback(async function (): Promise<number> {
    return Number(await getMiscKey('lastSync') ?? '0')
  }, [db])

  const setLastSync = useCallback(async function (timestamp: number) {
    return await setMiscValue('lastSync', timestamp.toString())
  }, [db])

  const getLastPlayed = useCallback(async function (): Promise<EpisodeData | undefined> {
    const r: { value: string }[] = await db.select(
      `SELECT value from misc
  ...
  const setLastPlaying = useCallback(async function (playingEpisode?: EpisodeData) {
  ...
  const getLastUpdate = useCallback(async function () {
    return Number(await getMiscKey('lastUpdate') ?? '0')
  }, [db])

  const setLastUpdate = useCallback(async function (timestamp: number) {
