...
export function useDownloads(db: Database) {
  ...
  const getLocalFile = useCallback(
    async function (episodeUrl: string) {
      const r: DownloadedEpisode[] = await db.select('SELECT * from downloads WHERE src = $1', [episodeUrl])
  ...
  const addToDownloadList = useCallback(
    async function (episode: EpisodeData, localFile: string) {
  ...
  const batchRemoveFromDownloadList = useCallback(
    async function (episodesSrc: string[]) {
      ...
      await db.execute(`DELETE FROM downloads WHERE src IN (${placeholders})`, [...episodesSrc])
    },
    [db],
  )
