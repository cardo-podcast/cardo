...
export function SyncButton() {
  ...
  return (
    <button
      className={`w-6 outline-none hover:text-accent-4 ${status === 'synchronizing' && 'animate-[spin_2s_linear_infinite_reverse]'}`}
      onClick={handleClick}
      title={error === '' ? t('sync') : error}
    >
      <svg
      ...
      >
        ...
        <circle
          ...
          cy="12"
          r="2"
          fill={status === 'error' ? 'red' : status === 'ok' ? 'green' : 'none'}
          stroke="none"
        />
